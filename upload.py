...
"""Tool for uploading diffs to the codereview app.

Usage summary: upload.py [options] [-- diff_options]

With Subversion, diff_options are passed to "svn diff".
With Git, diff_options are passed to "git diff".
"""
...
class VersionControlSystem(object):
  ...
  def GuessBase(self, required):
    ...
    """
    if required:
      ErrorExit("Cannot guess base URL; use the --local_base flag instead.")
...
class GitVCS(VersionControlSystem):
  ...
  def GenerateDiff(self, extra_args):
    ...
    gitdiff = RunShell("git diff", extra_args)
    svndiff = []
    filecount = 0
    filename = None
  ...
  def GetBaseFile(self, filename):
    ...
    if hash == "0" * 7:
      return ""  # No file to base against.
    else:
      return RunShell("git show", [hash])


def GuessVCS():
  """Guess and return the proper VersionControlSystem for this tree.
  ...
  # This will silently fail if you don't have git installed.
  if os.system("git rev-parse --is-inside-work-tree >/dev/null 2>&1") == 0:
    return GitVCS()

  ErrorExit("can't identify whether you're using svn or git")
